...
Until 2026-08-17 this section listed five equal "hard constraints". That was wrong
in a way that cost accuracy: it put the reason the product exists on the same
footing as modelling choices that were never anything but choices. The base model
now has one job — predict as well as the data allows — with two things it may
never do and one promise about the record. Full text and the banned-input table in
[`docs/constraints.md`](docs/constraints.md); the decision in
[ADR 0014](docs/adr/0014-the-liberation.md).

| | Untouchable | What it rules out |
|---|---|---|
| **1** | **No human polls. Ever.** | AP, Coaches and CFP rankings are comparison targets, never inputs — and never fitting targets either, which is the subtle version of the violation. There is no lever for this and there will not be one |
| **2** | **No future data. Ever.** | Every published number for a week is computable from games played before it. Once the objective is accuracy, walk-forward honesty stops being an ethic and becomes the definition of the measurement |
| — | **Every vintage is preserved** | Nothing published is edited in place. Every board stays up with the coefficients, `git_sha` and `config_hash` it ran under. This is the mechanism that let the freezes go |

**Everything else is a lever.** `cfbpoll levers` prints them: a plain-football
name, a published range, a default, and what measured that default. No lever
ships with blank evidence and a test asserts it. Conference identity is in the
registry, defaulted **off** — nothing in the model knows what a conference is, and
`cfbpoll audit-features` rebuilds every design matrix without `conference_game`
before every fit and requires bit-identical output, so that is a result rather
than a promise.

**The Poll makes three further promises** that are properties of what it is for:
it adjusts for who you played and where, simultaneously in one linear system; it
can be re-run with hindsight, which is why the estimator is a batch refit and not
an Elo; and every equation, constant and input is published, with every poll
traceable to the exact commit, config and data that produced it.
...
- The two untouchables, the lever registry, the headline promise and the
  banned-input table (`docs/constraints.md`)
