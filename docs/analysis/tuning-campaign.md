...
*Generated by `scripts/tuning_campaign.py` at commit `d706a0636b`; every number above is in `tuning-campaign.json`. Config hash `bd6a19c152f0222c` — the file as it stands NOW, carrying the frozen choice; the search cells were produced by explicit overrides on it rather than by editing it. Holdout touched: false.*
