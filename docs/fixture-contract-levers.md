...
That predicted 72 runs at a little over an hour. **The grid was then generated and
the real figure is 49 minutes**: 40.7 seconds per cell including its publish step,
measured across the 71 intervals between the first and last fit of the 2025 week 16
run. The single timed run above was cold and ran slow; plan on **45 to 60 minutes
per week of the season**, serial, and treat it as a background job rather than an
interactive one.

The whole published grid is **607,515 bytes**, counted off the tree: 72 documents
of roughly 8 KB plus a 32 KB manifest. A panel that prefetches everything pays
about three weeks of the poll.

**If the grid ever has to get cheaper, cut detents and not levers.** Dropping
`margin.c` to `{1, 18, 32, uncapped}` takes it to 48 runs and about 33 minutes
while leaving every lever in place and every shipped artifact still reproduced.
Dropping a lever instead would remove a whole question a reader can ask, and the
ordering lever in particular is the one argument this sport has most often.

**The two detents that would go first are `24` and `48`, and the grid says so
itself.** Both are one-knob edges, and on 2025 week 16 both came out
`convention`: `c = 24` at tau 0.9959 with nothing moved five places, `c = 48` at
tau 0.9882 with one team moved. They are the two settings a reader can move and
barely see, which is a finding worth publishing and also the reason they are the
...
### What it printed on the first published grid

```
OK    72 cells x weeks [16] under .../2025/lever-grid
      11 of them reproduce an already-published board and every shared row matches
      every cell carries the published poll's own evidence digest
```

All eight shipped playground variants came back identical to six decimals, tau and
verdict both: `margin-c-1` 0.972549 `dial`, `margin-c-18` 0.993464 `convention`,
`margin-c-uncapped` 0.974510 `dial`, `margin-beta-w-0` 0.984096 `dial`,
`margin-beta-w-3` 0.989325 `convention`, `margin-beta-w-12` 0.996950 `convention`,
`ordering-l4-resume` 0.979303 `dial`, `ordering-l4-resume-margin` 0.842484 `dial`.
The published cell's forty rows are field-for-field the published poll's.

Determinism, checked the way the project asks for it: one cell
(`c-18-bw-3-resume-margin`) ranked twice with the BLAS pin carried explicitly gave
byte-identical `poll.csv`, `poll.json`, `model_params.json`, `ratings_live.csv`,
`ratings_hindsight.csv` and `rank_intervals.csv`, and both published to the same
sha256 as the copy the generation run had already written.
...
- **The grid is regenerated wholesale or not at all.** There is no per-cell
  freshness marker, so a tree holding cells from two different generations would
  look exactly like a tree holding cells from one. The byte check catches a
  fabricated document and does not catch a stale one. If the grid ever spans more
  than one week or more than one archive version, it needs a run digest on every
  cell and a matching one on the manifest.
- **`n_moved_5_or_more` reaches 100 on the worst cell** (`c-uncapped-bw-0-resume-margin`,
  tau 0.7455), out of 136 teams compared. At that end of the grid the board is not
  a perturbation of the published poll in any useful sense, and a panel that draws
  a "what changed" diff should say so rather than render a hundred arrows.
