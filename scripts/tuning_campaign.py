...
def render(store: dict[str, Any]) -> None:  # noqa: PLR0915 - one long document
    ...
    lines: list[str] = [
        ...
        "> ",
        f"> **The campaign adopted C = {frozen['c']:g}, β_w = {frozen['beta_w']:g}, "
        f"garbage time = {frozen['garbage_time_mode']}, prediction compression "
        f"{'on' if frozen['prediction_compression'] else 'off'}** "
        f"([ADR 0007](../adr/0007-tuned-constants.md)). Tune-season MAE "
        f"{runs[('starting_values', 'tune')]['mae']:.4f} → "
        f"{runs[('frozen', 'tune')]['mae']:.4f}; 2024 validation "
        f"{runs[('starting_values', 'validate')]['mae']:.4f} → "
        f"{runs[('frozen', 'validate')]['mae']:.4f}. "
        + ("Adopted." if adopted else "REJECTED - the config keeps the starting values."),
        "> ",
        "> **The publication gate still fails all five decidable criteria**, and the "
        "campaign's central finding is why: the entire 416-cell search space spans "
        f"{max(r['mae'] for r in store['full_factorial']['cells']) - min(r['mae'] for r in store['full_factorial']['cells']):.3f} "  # noqa: E501
        "points of MAE and the gate needs 0.219. **The gap is structural, not a tuning "
        "problem.**",
        "> ",
        "> **The calibration miss is diagnosed and deliberately unfixed.** Neither named "
        "suspect - the normal tail, the single home-field constant - did it. The cause is "
        "under-dispersion of the point forecast, it replicates on 2024, and no "
        "pre-declared fix cleared the adoption bar. §5.8.",
        "> ",
        "> The protocol below is reproduced **verbatim from the commit that pre-registered "
        "it**, present tense and all. Where it says the constants are unfitted, that was "
        "true when it was written and is the point of writing it first.",
        "",
        "---",
        "",
        PROTOCOL.rstrip(),
