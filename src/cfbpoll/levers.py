...
class Lever:
    ...
    evidence: str
    #: Values the site may offer without a refit. Empty means continuous-only.
    sweep: tuple[float, ...] = ()
...
LEVERS: tuple[Lever, ...] = (
    ...
    Lever(
        ...
        plain=(
            ...
            "high as you like and margin counts all the way up."
        ),
        evidence="Fitted 2026-08-12 over a 416-cell factorial on the tune seasons. ADR 0007.",
        sweep=(18.0, 24.0, 32.0, 48.0, float("inf")),
        measured_effect=(
    ...
    Lever(
        ...
        default=1.0,
        plain=(
            "At 1 the table is sorted by how hard the schedule was to survive. At 0 it "
            "is sorted by the win-loss resume, which puts every unbeaten team above "
            "every team with a loss."
        ),
        evidence="ADR 0005 and docs/analysis/headline-ordering-study.md.",
        sweep=(0.0, 1.0),
        measured_effect=(
            "Under the resume ordering, retroactive re-ranking moved no unbeaten team "
            "a single place from week 11 of 2023 onward."
        ),
    ),
...
def validate(settings: dict[str, float]) -> dict[str, float]:
